...
app.get("/admin/add/question", async (req, res) => {
    ...
    request(options, function (err, response, body) {
        if (body.success) {
            res.render("question/questionAdd", {
                data: url,
                token: req.cookies.token,
            });
        } else {
                ...
                if (body.success) {
                    ...
                    request(options, function (err, response, body) {
                        if (body.length === 0) {
                            let errorData = {
                                message:
                                    "Question does not exist with the given id " + questionId,
                            };
    ...
    request(options, function (err, response, body) {
        ...
        request(options, function (err, response, body) {
            if (!body.success) {
                let errorData = {
                    message:
                        "Question update caused some error with the given id " + questionId,
                };
                return res.render("error", {
                    data: errorData,
                    imgUsername: req.cookies.username,
...
app.get("/admin/deletequestions/multiple", async (req, res) => {
    ...
    request(options, function (err, response, body) {
        res.render("question/deleteMultipleQuestions", {
            data: url,
            token: req.cookies.token,
        });
    });
        ...
        if (body.success) {
            ...
            request(options1, function (err, response, body) {
                ...
                request(options2, function (err, response, body) {
                    ...
                    let options3 = {
                        url: serverRoute + "/participations/" + req.params.contestId + "/" + req.cookies.username.toLowerCase(),
                        method: "get",
                        headers: {
                            authorization: req.cookies.token,
                        },
