...
export function colorFromHex(text) {
  ...
  switch (text.length) {
    ...
    case 7:
      return {
        red:
          (hexDigit(text.charCodeAt(1)) * 0x10 + hexDigit(text.charCodeAt(2))) /
          255.0,
        green:
          (hexDigit(text.charCodeAt(3)) * 0x10 + hexDigit(text.charCodeAt(4))) /
          255.0,
        blue:
          (hexDigit(text.charCodeAt(5)) * 0x10 + hexDigit(text.charCodeAt(6))) /
          255.0,
        alpha: 1,
      };
    case 9:
      return {
        ...
        blue:
          ...
          255.0,
        alpha:
          (hexDigit(text.charCodeAt(7)) * 0x10 + hexDigit(text.charCodeAt(8))) /
          255.0,
      };
  }
  return null;
...
export function getColorValue(node) {
  if (node.type === nodes.NodeType.HexColorValue) {
    ...
    try {
      var alpha =
        colorValues.length === 4 ? getNumericValue(colorValues[3], 1) : 1;
