...
function handleError2(jqXHR, textStatus, errorThrown) {
  showError(
    `${"Error"} (${jqXHR.status})`,
    `<pre>${jqXHR.responseJSON.message}</pre>`
  );
}

function handleMyResult2(data) {
  showError(
    `<h1 style="color: ${data.color}">${"Score - "}${data.score}</h1>`,
    `<pre style="font-size: large; color: ${data.color}">${data.message}</pre>`
  );
}
...
function fetchSubmission2(submission_token) {
  $.ajax({
    url:
      serverUrl + "/submissions/" + submission_token + "?base64_encoded=true",
    type: "GET",
    async: true,
    success: function (data, textStatus, jqXHR) {
      if (data.status.id <= 2) {
