...
app.post("/isOngoing", middleware.checkToken, async (req, res) => {
    ...
    try {
        ...
        let today = localServer ? `${year}-${month}-${day}` : `${year}-${day}-${month}`;
            ...
            if (accepted) {
                ...
                if (momentDate.isBefore(validTime) || req.decoded.admin) {
                    ...
                    if (result.token1 && result.token2 && result.token3) {
                        ...
                        try {
                            const participation = await participations.acceptSubmission(result);
...
const getSubmissionStatus = (options) => {
...
const getSubmissionToken = (options) => {
    return axios(options)
        .then((response) => {
